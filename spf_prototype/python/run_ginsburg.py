...
def _mat_score(sp_path, tally_name, mat_id, score_substrs):
    """Mean+std of a tally for one material/cell id and a score whose label
    contains any of score_substrs. Reads tally.mean/std_dev arrays directly
    (get_pandas_dataframe() is broken on this pandas version). Sums over nuclide
    bins; filter bins are C-ordered (first filter slowest). (nan,nan) if absent."""
    import numpy as np
    import openmc
    with openmc.StatePoint(sp_path) as s:
        t = s.get_tally(name=tally_name)
        scores = list(t.scores)
        s_idx = next((i for i, sc in enumerate(scores)
                      if any(ss in str(sc) for ss in score_substrs)), None)
        if s_idx is None:
        ...
        for f in reversed(t.filters):
            ...
            for j, b in enumerate(bins):
                try:
                    if int(np.atleast_1d(b).ravel()[0]) == int(mat_id):
        ...
        if t.filters and not found:
            return float("nan"), float("nan")
        m = t.mean.reshape(mult, -1, len(scores))
        sd = t.std_dev.reshape(mult, -1, len(scores))
