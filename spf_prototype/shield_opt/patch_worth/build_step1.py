...
VARIANT = sys.argv[1]
assert VARIANT in ("placed", "uniform", "multicoil", "multicoil_greedy"), VARIANT
...
if VARIANT in ("multicoil", "multicoil_greedy"):
    dd = np.load(f"{export_dir}/qh_step1_deltas_multicoil.npz")
    # multicoil        -> delta_multicoil   (proportional/spread; the DOSED variant that LOST)
    # multicoil_greedy -> delta_multi_greedy (bang-bang concentrate on the collective inboard;
    #                     same 630.7 cm budget). Tests concentrate-vs-spread at fixed material.
    delta = dd["delta_multi_greedy"] if VARIANT == "multicoil_greedy" else dd["delta_multicoil"]
else:
