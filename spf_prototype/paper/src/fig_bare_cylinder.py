...
ax.plot(s, inner, "o-", color="0.15", label="inboard (convex)")
ax.plot(s, outer, "s--", color="0.55", label="outboard (concave)")
ss = np.linspace(0, 1, 50)
ax.plot(ss, inner[-1] * ss**2, ":", color="0.15", lw=1.0, label="~ s^2")
ax.set_xlabel("plasma shaping amplitude s")
ax.set_ylabel("analytic point-patch error |A/iso - ray|")
ax.set_title("Filamentary analytic breaks down with shaping")
