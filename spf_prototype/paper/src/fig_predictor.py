...
ax[0].set_title(r"headroom: $\rho=+0.53$ (n=15)", fontsize=9)
ax[1].scatter(Q, Y, s=42, marker="s", facecolor="none", edgecolor="0.4")
ax[1].set_xlabel("plasma shape (nematic Q)")
ax[1].set_title(r"shape: $\rho=+0.40$, n.s.", fontsize=9)
for a in ax:
    a.axhline(0, color="0.8", lw=0.8)
    a.set_ylim(-1, 18)
fig.suptitle("SPF benefit is weak (median 4%) and not forecastable from plasma shape", fontsize=10)
