...
for real stellarators and the local-versus-global result. Section~\ref{sec:companion}
...
source is what the analytic method cannot do, and the first of these is neutron transport.
To isolate the effect of transport itself we deliberately hold the geometry fixed and
simple: a square-cross-section (box) torus with a radially layered blanket, so that the
wall is identical to the free-streaming cross-checks and the only change is the material.
Any departure from the free-streaming steering is then attributable to scattering alone,
not to a change of shape, and the radial blanket stack gives every neutron a well-defined,
unambiguous path through the layers for a clean energy and breeding accounting---the
conformal, poloidally- and toroidally-varying wall of \S\ref{sec:conformal} would confound
the transport effect we want to measure here with the geometric load variation we measure
there. Concretely, replacing the void interior and vacuum walls with an ARC-class layered
blanket---a $1$~mm tungsten first wall, $1$~cm of Fe-9Cr structural steel, a $1$~cm
beryllium multiplier, and $40$~cm of FLiBe breeder---turns the first-flight NWL into the
transported flux and heating that a reactor blanket actually sees. The polarized angular
...
studies (\S\ref{sec:transport}--\ref{sec:rate}) use a layered box-torus blanket rather than
the conformal stellarator wall of \S\ref{sec:conformal}; the two are used at the fidelity
that isolates their respective effects---the box-torus for a clean volumetric energy and
breeding accounting, the conformal wall for the geometric load pattern---but a unified
conformal, scattering run is the natural next step and is expected to inherit the
$\sim\!3\times$ dilution. The steering results are monoenergetic at $14.1$~MeV; a
